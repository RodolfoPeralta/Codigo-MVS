 ...
 * Modo 2: Mediciones
 * Se envía una portadora de 40KHz y se activa una interrupción si el Rx recibe la portadora 
   o si se alcanza el TIME OUT.
 ...
 * Se cambió como se trabaja con los contadores.
 ...
 * Se cambio la interrupción de un FALLING a un LOW, dado que no funcionaba muy bien con FALLING.
 * Compatible con Tx v8 y Rx v8 (diseño circuital).
 ...
 * 
 * Modo 2: mediciones
 * Se reinicia el Timer, para este caso se utiliza el Timer1 que es de 16 bits y trabaja a una 
   frecuencia de 16 MHz, con unar resolución de 62.5ns.
...
const unsigned long timeOut = 1600000;                                // Después de este tiempo se tomará como que no hubo eco (está en ticks).
...
void setup() {
  ...
  if (!SD.begin(SSpin)) {
    ...
    delay(2000);
    return; // Si falla la inicialización se termina el programa.
  }
  lcd.print("SD OK");
  delay(2000);
  ...
  switch(modValue) {
      ...
      case 2:
        ...
        for (int i = 0 ; i < measureQuantity ; i++) {       // Iteración para cada medición.
          ...
          digitalWrite(CS, LOW);  // Dejo de transmitir.
          delay(2000);            // Para mi el bug se arregla con esto. Creo que cuando volvemos a medir, si justo quedó un rebote en el canal el circuito lo agarra antes de dejar de transmitir y ahí entra en un loop.
        }

        //************ CALCULOS ************//
  
        for (int i=0 ; i < measureQuantity ; i++) {
          measureTime[i] -= 125;    //Resto el retardo dado por el error de desfase de portadora, y el que agrega el comparador de umbral.
        ...
        lcd.clear();
        lcd.setCursor(0,0);
        lcd.print("Tipo de Medicion:");
        lcd.setCursor(0,1);
        lcd.print("1: Velocidad");
        lcd.setCursor(0,2);
        lcd.print("2: Profundidad");
        lcd.setCursor(10,3);
        lcd.print(modValue);
        ...
        switch(modValue) {
          case 1: 
            velocity = 0.35 / (0.000001*measureAverage);    //velocidad = distancia/tiempo. distancia son los 35cm=0.35m del soporte y el tiempo el dado por el promedio de las mediciones.
            lcd.clear();
            lcd.setCursor(0,0);
            lcd.print("Velocidad Promedio:");
            lcd.setCursor(5,2);
            lcd.print(velocity);
            lcd.print(" m/s");
          ...
          case 2:
            depth = ((1480*measureAverage)/2.0)*100;    //profundidad=(velocidad*tiempo)/2, el 100 es para pasar de metros a centimetros.
            lcd.clear();
            lcd.setCursor(0,0);
        ...
        if (archivo) {                                            // De abrirse correctamente, comienza a escribirse un archivo en la tarjeta SD.
          ...
          archivo.print(velocity);
          archivo.println();
          archivo.print("Profundidad: ");
          archivo.print(depth);
          archivo.println();
